...
const getMyLocation = async (req, res) => {
	...
	return res.json({ location: { latitude: location.latitude, longitude: location.longitude, accuracy: location.accuracy, capturedAt: location.capturedAt } });
};
...
const saveMyLocation = async (req, res) => {
	...
	const location = { latitude: coordinates.latitude, longitude: coordinates.longitude, accuracy: parsedAccuracy, capturedAt: date };
