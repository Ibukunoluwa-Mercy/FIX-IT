...
const { sendVerificationEmail, sendWelcomeEmail, sendLoginEmail } = require('../services/emailService');
const {
	ROLE_MAP,
	emailPattern,
	normalizeText,
...
const createAdmin = async (req, res) => {
	...
	const email = normalizeText(req.body.email).toLowerCase();
	const password = typeof req.body.password === 'string' ? req.body.password : '';
	if (!fullName || !email || !password || !emailPattern.test(email) || password.length < 8) {
		return res.status(400).json({ message: 'Valid full name, email, and password are required' });
	}
